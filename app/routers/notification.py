...
@router.get("/")
async def get_notifications(credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    notifications = await db.notifications.find({"user_id": ObjectId(user_id)})
...
async def read_by_id(body: NotificationReadBody, credentials: HTTPAuthorizationCredentials = Security(security)):
    token = credentials.credentials
    userctx = decodeJWT(token)
    if not userctx:
        raise HTTPException(status_code=403, detail="Not authorized")
